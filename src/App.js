...
function Welcome(props) {
	return (
		<div>
			<h2>Super Mario</h2>
			<p>The aim is to eat all in the mashrooms in 64 steps.
       The default size of the maze is 10 x 10,
       but you can change it by reloading or entering the height and width in the prompt.</p>
		</div>
		)
}
...
function ScoreCard(props) {
	...
	total_score.innerHTML = max_mashroom - document.getElementsByClassName('active').length
}
...
function checkFinish() {
	...
	if(check.length === 0){
		let game_completed = window.confirm("hurray!!! You have finished the game in "+ no_of_moves + " moves.");
...
let Box = createReactClass({
	getInitialState: function() {
		...
		for(let i=1; i<=this.props.matrix; i++){
			game_cells.push( <Cell key={i} id={i} cells={game_cells} /> )
			items.push(i)
		}
		return {cells: game_cells}
	},
	render: function() {
		return (
...
class App extends Component {
	...
	handleLoad(width,height){
		let matrix = document.getElementById('game')
		...
		for (let i = 0; i < truncated_data.length; i++) {
			...
			elem_position.innerHTML="<img src='mashroom.jpeg' alt='mario' class='maze-image'/>";
